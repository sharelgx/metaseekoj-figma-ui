...
export default function ACMRank() {
  ...
  const getRankData = async () => {
    setLoading(true)
    try {
      const data = await rankAPI.getACMRank({
        offset: (page - 1) * limit,
        limit: limit
      })
  ...
  const getACRate = (ac: number, total: number) => {
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h2 style={{ margin: 0, fontSize: '24px', color: '#303133' }}>ACM 排名榜</h2>
            </div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#fafafa', borderBottom: '2px solid #e8eaec' }}>
                  ...
                  <th style={{ padding: '12px', textAlign: 'center', color: '#909399', fontSize: '14px', width: '120px' }}>AC数</th>
                  <th style={{ padding: '12px', textAlign: 'center', color: '#909399', fontSize: '14px', width: '120px' }}>提交数</th>
                  <th style={{ padding: '12px', textAlign: 'center', color: '#909399', fontSize: '14px', width: '120px' }}>AC率</th>
                </tr>
              </thead>
              <tbody>
                {dataRank.map((item, index) => {
                  ...
                  return (
                    <tr key={index} style={{ borderBottom: '1px solid #e8eaec' }}>
                      ...
                      <td style={{ padding: '16px 12px', textAlign: 'center', color: '#67c23a', fontSize: '16px', fontWeight: 600 }}>
                        {item.accepted_number}
                      </td>
                      <td style={{ padding: '16px 12px', textAlign: 'center', color: '#606266', fontSize: '14px' }}>
                        {item.submission_number}
                      </td>
                      <td style={{ padding: '16px 12px', textAlign: 'center', color: '#409eff', fontSize: '14px', fontWeight: 600 }}>
                        {getACRate(item.accepted_number, item.submission_number)}
                      </td>
                    </tr>
                  )
                })}
